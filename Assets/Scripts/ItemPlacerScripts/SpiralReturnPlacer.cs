...
public class SpiralReturnPlacer : PipeItemGenerator {
	...
	public override void GenerateItems(Pipe pipe){
		...
		for (int i = 0; i < pipe.CurveSegmentCount/2; i++) {
		...
		for (int i = pipe.CurveSegmentCount/2; i < pipe.CurveSegmentCount; i++) {
			PipeItem item = Instantiate<PipeItem> (
				itemPrefabs [Random.Range (0, itemPrefabs.Length)]);
			float pipeRotation =
				(start + i * direction) * 360f / pipe.pipeSegmentCount;
