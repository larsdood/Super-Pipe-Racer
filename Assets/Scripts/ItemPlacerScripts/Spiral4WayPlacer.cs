...
public class Spiral4WayPlacer : PipeItemGenerator {
	...
	public override void GenerateItems(Pipe pipe){
		...
		for (int i = pipe.CurveSegmentCount/3; i < pipe.CurveSegmentCount*2/3; i++) {
			PipeItem item = Instantiate<PipeItem> (
				itemPrefabs [Random.Range (0, itemPrefabs.Length)]);
			float pipeRotation =
				(start + i * direction) * 360f / pipe.pipeSegmentCount;
		...
		direction = direction < 0f ? 1f : -1f;
		for (int i = pipe.CurveSegmentCount*2/3; i < pipe.CurveSegmentCount; i++) {
			PipeItem item = Instantiate<PipeItem> (
				itemPrefabs [Random.Range (0, itemPrefabs.Length)]);
			float pipeRotation =
				(start + i * direction) * 360f / pipe.pipeSegmentCount;
		...
		/*direction = direction < 0f ? 1f : -1f;
		for (int i = pipe.CurveSegmentCount*3/4; i < pipe.CurveSegmentCount; i++) {
			PipeItem item = Instantiate<PipeItem> (
				itemPrefabs [Random.Range (0, itemPrefabs.Length)]);
			float pipeRotation =
				(start + i * direction) * 360f / pipe.pipeSegmentCount;
